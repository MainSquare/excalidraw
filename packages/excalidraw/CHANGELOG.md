...
We've transitioned from `UMD` to `ESM` bundle format. Our new `dist` folder inside `@mainsquare/excalidraw` package now contains only bundled source files, making any dependencies tree-shakable. The package comes with the following structure:
...
import type { ExcalidrawImperativeAPI } from "@mainsquare/excalidraw/types";
```
...
<link
  rel="stylesheet"
  href="https://esm.sh/@mainsquare/excalidraw@0.18.0/dist/dev/index.css"
/>
...
<script type="module">
  ...
  import * as ExcalidrawLib from "https://esm.sh/@mainsquare/excalidraw@0.18.0/dist/dev/index.js?external=react,react-dom";
</script>
```
...
// after
import { CaptureUpdateAction } from "@mainsquare/excalidraw";
updateScene({
  elements,
  appState,
...
- Prefer user defined coordinates and dimensions when creating a frame using [`convertToExcalidrawElements`](https://docs.excalidraw.com/docs/@mainsquare/excalidraw/api/excalidraw-element-skeleton#converttoexcalidrawelements) [#8517](https://github.com/excalidraw/excalidraw/pull/8517)
...
- Export types for @mainsquare/excalidraw-utils [#7736](https://github.com/excalidraw/excalidraw/pull/7736)
...
- Export [`getCommonBounds`](https://docs.excalidraw.com/docs/@mainsquare/excalidraw/api/utils#getcommonbounds) helper from the package [#7247](https://github.com/excalidraw/excalidraw/pull/7247).
...
- The `Ref` support has been removed in v0.17.0 so if you are using refs, please update the integration to use the [`excalidrawAPI`](http://localhost:3003/docs/@mainsquare/excalidraw/api/props/excalidraw-api) [#7251](https://github.com/excalidraw/excalidraw/pull/7251).
...
- [`useDevice`](https://docs.excalidraw.com/docs/@mainsquare/excalidraw/api/utils#usedevice) hook's return value was changed to differentiate between `editor` and `viewport` breakpoints. [#7243](https://github.com/excalidraw/excalidraw/pull/7243)
...
- Added [`props.validateEmbeddable`](https://docs.excalidraw.com/docs/@mainsquare/excalidraw/api/props#validateembeddable) to customize embeddable src url validation. [#6691](https://github.com/excalidraw/excalidraw/pull/6691)
- Add support for `opts.fitToViewport` and `opts.viewportZoomFactor` in the [`ExcalidrawAPI.scrollToContent`](https://docs.excalidraw.com/docs/@mainsquare/excalidraw/api/props/excalidraw-api#scrolltocontent) API. [#6581](https://github.com/excalidraw/excalidraw/pull/6581).
...
- Use subdirectory for @mainsquare/excalidraw size limit [#6787](https://github.com/excalidraw/excalidraw/pull/6787)
...
- [`ExcalidrawAPI.scrollToContent`](https://docs.excalidraw.com/docs/@mainsquare/excalidraw/api/props/excalidraw-api#scrolltocontent) has new opts object allowing you to fit viewport to content, and animate the scrolling. [#6319](https://github.com/excalidraw/excalidraw/pull/6319)
...
- [`restoreElements`](https://docs.excalidraw.com/docs/@mainsquare/excalidraw/api/utils/restore#restoreelements) API now takes an optional parameter `opts` which currently supports the below attributes

```js
{ refreshDimensions?: boolean, repairBindings?: boolean }
```

The same `opts` param has been added to [`restore`](https://docs.excalidraw.com/docs/@mainsquare/excalidraw/api/utils/restore#restore) API as well.

For more details refer to the [docs](https://docs.excalidraw.com)

#### BREAKING CHANGE

- The optional parameter `refreshDimensions` in [`restoreElements`](https://docs.excalidraw.com/docs/@mainsquare/excalidraw/api/utils/restore#restoreelements) has been removed and can be enabled via `opts`
...
- Added exportPadding to PNG (blob) export in @mainsquare/excalidraw-utils [#5626](https://github.com/excalidraw/excalidraw/pull/5626)
...
- Export types for the package so now it can be used with typescript [#3337](https://github.com/excalidraw/excalidraw/pull/3337). The types are available at `@mainsquare/excalidraw/types`.
...
- Expose `window.EXCALIDRAW_ASSET_PATH` which host can use to load assets from a different URL. By default it will be loaded from `https://unpkg.com/@mainsquare/excalidraw{currentVersion}/dist/`[#3068](https://github.com/excalidraw/excalidraw/pull/3068).
...
First release of `@mainsquare/excalidraw`
