...
The need for these two steps is due to the [@mainsquare/excalidraw](/docs/@mainsquare/excalidraw/installation) being a **UMD** build so we currently cannot import the `convertToExcalidrawElements()` util alone, until we support a tree-shakeable **ESM** build.
...
import { convertToExcalidrawElements}  from "@mainsquare/excalidraw"
try {
