...
Further, if the sidebader doesn't comfortably fit in the editor, it won't be dockable. To decide the breakpoint for docking you can use [UIOptions.dockedSidebarBreakpoint](/docs/@mainsquare/excalidraw/api/props/ui-options#dockedsidebarbreakpoint).
...
You can use the [`ref.toggleSidebar({ name: "custom" })`](/docs/@mainsquare/excalidraw/api/props/excalidraw-api#toggleSidebar) api to control the sidebar, but we export a trigger button to make UI use cases easier.
