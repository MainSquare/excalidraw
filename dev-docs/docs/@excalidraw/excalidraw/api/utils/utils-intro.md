---
slug: /@mainsquare/excalidraw/api/utils
---

# Utils

These are pure Javascript functions exported from the @mainsquare/excalidraw [`@mainsquare/excalidraw`](https://npmjs.com/@mainsquare/excalidraw). If you want to export your drawings in different formats eg `png`, `svg` and more you can check out [Export Utilities](/docs/@mainsquare/excalidraw/API/utils/export). If you want to restore your drawings you can check out [Restore Utilities](/docs/@mainsquare/excalidraw/API/utils/restore).
...
```js
import { serializeAsJSON } from "@mainsquare/excalidraw";
```
...
```js
import { serializeLibraryAsJSON } from "@mainsquare/excalidraw";
```
...
```js
import { isInvisiblySmallElement } from "@mainsquare/excalidraw";
```
...
```js
import { loadFromBlob } from "@mainsquare/excalidraw";
...
```js
import { loadLibraryFromBlob } from "@mainsquare/excalidraw";
```
...
```js showLineNumbers
import { loadSceneOrLibraryFromBlob, MIME_TYPES } from "@mainsquare/excalidraw";
...
```js
import { getFreeDrawSvgPath } from "@mainsquare/excalidraw";
```
...
```js
import { isLinearElement } from "@mainsquare/excalidraw";
```
...
```js
import { getNonDeletedElements } from "@mainsquare/excalidraw";
```
...
```js
import { mergeLibraryItems } from "@mainsquare/excalidraw";
```
...
```js
import { parseLibraryTokensFromUrl } from "@mainsquare/excalidraw";
```
...
```js
import { useHandleLibrary } from "@mainsquare/excalidraw";
...
```js
import { getSceneVersion } from "@mainsquare/excalidraw";
```
...
```js
import { sceneCoordsToViewportCoords } from "@mainsquare/excalidraw";
```
...
```js
import { viewportCoordsToSceneCoords } from "@mainsquare/excalidraw";
```
...
```js
import { defaultLang, languages, useI18n } from "@mainsquare/excalidraw";
```
...
List of supported language codes. You can pass any of these to `Excalidraw`'s [`langCode` prop](/docs/@mainsquare/excalidraw/api/props/#langcode).
...
```js
import { getCommonBounds } from "@mainsquare/excalidraw";
```
...
```js
import { elementsOverlappingBBox } from "@mainsquare/excalidraw";
```
...
```js
import { isElementInsideBBox } from "@mainsquare/excalidraw";
```
...
```js
import { elementPartiallyOverlapsWithOrContainsBBox } from "@mainsquare/excalidraw";
```
